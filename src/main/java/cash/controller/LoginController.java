...
public class LoginController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 아이디 저장 쿠키
		// 로그인 정보 세션 저장 + 로그인 id를 쿠키(만료기간설정)에 저장 -> loginMember에 저장 - dao호출 반환값 getId
		// 로그인 페이지에 아이디저장이 체크되어있으면(test) idSave값이 넘어왔다면
		if(request.getParameter("idSave")!= null) { // idSave가 null이 아니라면
			Cookie loginIdcookie = new Cookie("loginMember",loginMember.getMemberId()); // 쿠키가 하나의 map이다 매개변수필요 string,string (키값) 저장
			loginIdcookie.setMaxAge(60*60*24); // 기간 : 초단위로 계산 ex) 60*60*24 = 하루
			response.addCookie(loginIdcookie); // 응답. 쿠키정보를 사용자에게 전송
		}else {
			Cookie loginIdcookie = new Cookie("loginMember",null);
